...
export function MetricCard({ label, value, sub, trend, trendUp, accentColor, glowColor, iconEl }: {
    ...
    return (
        <div className="flex-1 rounded-2xl p-4 relative overflow-hidden" style={{ background: "#0c1128", border: "1px solid #151d35" }}>
            <div className="absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl opacity-10 pointer-events-none" style={{ background: glowColor, transform: "translate(30%,-30%)" }} />
            <div className="flex items-start justify-between mb-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: `${accentColor}18`, border: `1px solid ${accentColor}30`, color: accentColor }}>
                    {iconEl}
                </div>
                {trend && (
                    <span className="text-[11px] font-mono font-semibold px-2 py-0.5 rounded-full"
                        style={{ background: trendUp ? "rgba(34,197,94,0.1)" : "rgba(239,68,68,0.1)", color: trendUp ? "#22c55e" : "#ef4444", border: `1px solid ${trendUp ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)"}` }}>
                        {trendUp ? "↑" : "↓"} {trend}
                    </span>
                )}
            </div>
            <div className="font-mono text-[28px] font-bold leading-none mb-1" style={{ color: accentColor }}>{value}</div>
            <div className="text-[12px] font-semibold text-white mb-0.5">{label}</div>
            <div className="text-[11px]" style={{ color: "#4a5a80" }}>{sub}</div>
        </div>
    );
}
    ...
    return (
        <div className="shrink-0 flex items-center justify-between px-6 py-3" style={{ borderBottom: "1px solid #151d35", background: "#07091a" }}>
            <div>
                ...
                <p className="text-[11px] font-mono mt-0.5" style={{ color: "#4a5a80" }}>{sub}</p>
            </div>
...
export function PrimaryBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
    ...
    return (
        <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
            ...
            style={{ background: hov ? "#38bdf8" : "#22d3ee", color: "#07091a", boxShadow: "0 4px 16px rgba(34,211,238,0.25)" }}>
            {children}
        </button>
    );
}