...
export function WarehouseGrid({ robots }: { robots: RobotPos[] }) {
    const cellTypeStyles: Record<string, { fill: string; stroke: string }> = {
        floor: { fill: "#060d1f", stroke: "#0d1634" }, wall: { fill: "#0a1128", stroke: "#151d35" },
        shelf: { fill: "#0f1a30", stroke: "#1a2a4a" }, pickup: { fill: "#1c3a6e", stroke: "#3b82f6" },
        delivery: { fill: "#1a4025", stroke: "#22c55e" }, charge: { fill: "#3a2e00", stroke: "#f59e0b" },
        dock: { fill: "#2d1a5e", stroke: "#8b5cf6" },
    };

    return (
        <svg width="100%" height="100%" viewBox={`0 0 ${GW} ${GH}`} style={{ background: "#060d1f", display: "block" }} preserveAspectRatio="xMidYMid meet">
            <defs>
                <pattern id="dots" x="0" y="0" width={CELL} height={CELL} patternUnits="userSpaceOnUse"><circle cx="0" cy="0" r="0.8" fill="#0d1634" /></pattern>
                ...
                <filter id="glow-red" x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur stdDeviation="2" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
                ...
                <marker id="arrow" markerWidth="4" markerHeight="4" refX="2" refY="2" orient="auto"><path d="M0,0 L4,2 L0,4 Z" fill="#22d3ee" opacity="0.5" /></marker>
            </defs>
            ...
            {GRID.map((row, ri) => row.map((cell, ci) => {
                ...
                const shelfColor = cell.color || style.fill;
                return (
                    <g key={`${ri}-${ci}`}>
                        <rect x={x} y={y} width={CELL} height={CELL} fill={cell.type === "shelf" ? shelfColor : style.fill} stroke={style.stroke} strokeWidth="0.5" />
                        {cell.type === "shelf" && (<><rect x={x + 3} y={y + 4} width={CELL - 6} height={6} rx="1" fill={style.stroke} opacity="0.5" /><rect x={x + 3} y={y + 14} width={CELL - 6} height={6} rx="1" fill={style.stroke} opacity="0.5" /><rect x={x + 3} y={y + 24} width={CELL - 6} height={6} rx="1" fill={style.stroke} opacity="0.5" /></>)}
                        {(cell.type === "pickup" || cell.type === "delivery" || cell.type === "charge" || cell.type === "dock") && (<><rect x={x + 2} y={y + 2} width={CELL - 4} height={CELL - 4} rx="3" fill={style.fill} stroke={style.stroke} strokeWidth="1.5" /><text x={x + CELL / 2} y={y + CELL / 2 + 4} textAnchor="middle" fontSize="8" fontFamily="monospace" fontWeight="bold" fill={style.stroke}>{cell.label}</text></>)}
                    </g>
                );
            }))}
            {ZONE_DEFS.map(({ label, r, c, color }) => (<text key={label} x={c * CELL + 4} y={r * CELL + 12} fontSize="9" fontFamily="monospace" fontWeight="800" fill={color} opacity="0.7" letterSpacing="1">{label.toUpperCase()}</text>))}
            {robots.filter(rb => rb.missionId && rb.mqtt === "ONLINE").map(rb => (<line key={`path-${rb.id}`} x1={rb.x * CELL + CELL / 2} y1={rb.y * CELL + CELL / 2} x2={rb.tx * CELL + CELL / 2} y2={rb.ty * CELL + CELL / 2} stroke={rb.color} strokeWidth="1" strokeDasharray="4 3" opacity="0.25" markerEnd="url(#arrow)" />))}
            {robots.map((rb) => {
                ...
                return (
                    <g key={rb.id} filter={`url(#${filterId})`}>
                        {rb.mqtt === "ONLINE" && (<circle cx={cx} cy={cy} r={R + 5} fill="none" stroke={rb.color} strokeWidth="0.8" opacity="0.3"><animate attributeName="r" values={`${R + 3};${R + 10};${R + 3}`} dur="2s" repeatCount="indefinite" /><animate attributeName="opacity" values="0.4;0;0.4" dur="2s" repeatCount="indefinite" /></circle>)}
                        <rect x={cx - R} y={cy - R} width={R * 2} height={R * 2} rx="4" fill={rb.mqtt === "OFFLINE" ? "#1a0a0a" : "#0a1628"} stroke={rb.color} strokeWidth="1.5" />
