...
export function LoginPage({ onLogin }: { onLogin: (role: "operator" | "admin") => void }) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4" style={{ background: "var(--background)" }}>
            <div className="w-full max-w-md rounded-2xl p-8" style={{ background: "var(--surface-2)", border: "1px solid #151d35" }}>
                <div className="flex flex-col items-center mb-8">
                    ...
                    <h1 className="text-xl font-bold text-foreground">SmartFleet WMS</h1>
                    <p className="text-[12px] mt-1" style={{ color: "var(--text-faint)" }}>Sign in to your account</p>
                </div>

                <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                        <label className="block text-[11px] font-mono mb-1.5" style={{ color: "var(--text-faint)" }}>EMAIL</label>
                        <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                            className="w-full px-3 py-2 rounded-lg text-[13px] text-foreground focus:outline-none"
                            style={{ background: "var(--surface-1)", border: "1px solid #151d35" }} placeholder="operator@smartfleet.com" required />
                    </div>
                    <div>
                        <label className="block text-[11px] font-mono mb-1.5" style={{ color: "var(--text-faint)" }}>PASSWORD</label>
                        <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                            className="w-full px-3 py-2 rounded-lg text-[13px] text-foreground focus:outline-none"
                            style={{ background: "var(--surface-1)", border: "1px solid #151d35" }} placeholder="••••••••" required />
                    </div>
