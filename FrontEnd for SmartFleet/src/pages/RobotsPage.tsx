    ...
    return (
        <div className="flex flex-col h-full p-6 overflow-y-auto" style={{ background: "var(--background)" }}>
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-[18px] font-bold text-foreground">Fleet Management</h1>
                    <p className="text-[12px] font-mono mt-1" style={{ color: "var(--text-faint)" }}>Monitor and control all autonomous mobile robots.</p>
                </div>
            ...
            <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))" }}>
                {robots.map(rb => {
                    ...
                    return (
                        <div key={rb.id} className="rounded-2xl p-5 flex flex-col gap-4" style={{ background: "var(--surface-2)", border: `1px solid ${online ? "#151d35" : "rgba(239,68,68,0.25)"}` }}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="font-mono text-[14px] font-bold text-foreground">{rb.id}</div>
                                        <div className="text-[11px]" style={{ color: "var(--text-faint)" }}>{rb.model}</div>
                                    </div>
                                </div>
                            ...
                            <div className="rounded-xl px-4 py-3" style={{ background: "var(--surface-1)", border: "1px solid #151d35" }}>
                                <div className="text-[10px] font-mono mb-1" style={{ color: "var(--text-faint)" }}>CURRENT MISSION</div>
                                <div className="font-mono text-[12px] font-semibold" style={{ color: rb.missionId ? "#22d3ee" : "#2d3f66" }}>
                                    {rb.missionId || "— IDLE —"}
                                </div>
                                <div className="text-[10px] font-mono mt-1" style={{ color: "var(--text-faint)" }}>Zone: {rb.zone}</div>
                            </div>

                            <div>
                                <div className="flex justify-between mb-1.5">
                                    <span className="text-[11px] font-mono" style={{ color: "var(--text-faint)" }}>BATTERY</span>
                            ...
                            <div className="flex items-center justify-between pt-2" style={{ borderTop: "1px solid #151d35" }}>
                                <span className="text-[11px] font-mono" style={{ color: "var(--text-faint)" }}>MAINTENANCE MODE</span>
