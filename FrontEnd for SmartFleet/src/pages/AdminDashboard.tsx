...
export function AdminDashboard() {
    ...
    return (
        <div className="flex flex-col h-full" style={{ background: "#070d1e" }}>
            ...
            <div className="flex items-center gap-3 px-6 py-3 shrink-0" style={{ borderBottom: "1px solid #151d35" }}>
                <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "#4a5a80" }}><Icon d={IC.search} size={14} /></span>
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search robots..." className="pl-9 pr-4 py-2 rounded-xl text-[13px] text-white focus:outline-none" style={{ background: "#0c1128", border: "1px solid #151d35", width: 280 }} />
                </div>
                <span className="font-mono text-[11px]" style={{ color: "#2d3f66" }}>{filteredRobots.length} robots registered</span>
            </div>

            <div className="flex-1 overflow-hidden m-6 rounded-2xl" style={{ background: "#0c1128", border: "1px solid #151d35" }}>
                <div className="overflow-x-auto h-full">
                    <table className="w-full border-collapse min-w-[800px]">
                        <thead>
                            <tr style={{ borderBottom: "1px solid #151d35", background: "#07091a" }}>
                                {["Robot ID", "Model Type", "Zone", "Battery", "MQTT", "Maintenance", "Actions"].map(h => (
                                    <th key={h} className="px-4 py-3 text-left font-mono text-[10px] font-semibold tracking-wider" style={{ color: "#2d3f66" }}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {filteredRobots.map(r => (
                                <tr key={r.id} style={{ borderBottom: "1px solid #0d1634" }}>
                                    ...
                                    <td className="px-4 py-3 text-[12px] text-white">{r.model}</td>
            ...
            {toast && (
                <div className="fixed bottom-6 right-6 p-3 rounded-lg text-[12px] font-mono flex items-center gap-2 shadow-xl" style={{ background: "#0c1128", border: "1px solid rgba(34,197,94,0.3)", color: "#4ade80", zIndex: 100 }}>
